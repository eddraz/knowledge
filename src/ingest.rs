...
use crate::db::{delete_document, insert_chunk, insert_document};
...
pub async fn ingest(
    ...
    let chunks = chunk_text(text, cfg.chunk_target_chars, cfg.chunk_overlap_chars);
    if chunks.is_empty() {
    ...
    let doc_id = insert_document(&tx, source, &new_hash)?;
